...
- **端到端断言**：用 puppeteer-core 驱动本机 Chrome，覆盖「封面 → 验证码错误拦截 → 正确码放行 → 一口气答完 16 题（断言中途不再弹验证码）→ 结果页 → 再测一次 → 断言**直接进入新一轮且不再经过验证码屏**、连测三轮题面各不相同」，并断言**验证码不出现在页面可见文本中**、**每轮 MAXRAW 恒定**、**选项无过渡动画**、**0 JS 错误**。刷新页面后则需重新验证一次。
